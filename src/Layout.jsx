        ...
        <div className="mx-auto px-6 flex items-center justify-between h-16">
          ...
          <div className="hidden md:flex items-center gap-6">
            ...
            <div className="relative">
              ...
              <AnimatePresence>
                {servicesOpen && (
                  ...
                  >
                    {services.map((s) => (
                      <Link key={s.id} to={s.href || '#'} className="block px-3 py-2 text-sm">
                        {s.title}
                      </Link>
                    ))}
                  </motion.div>
            ...
            <button
              ...
              className="text-sm bg-blue-600 px-3 py-1 rounded-md"
            >
              Request Demo
            </button>
          </div>
                  ...
                  <AnimatePresence>
                    {servicesOpen && (
                      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                        {services.map((s) => (
                          <Link key={s.id} to={s.href || '#'} className="block px-2 py-1">
                            {s.title}
                          </Link>
                        ))}
                      </motion.div>
...
export default function Layout({ children }) {
  ...
  return (
    <div className="bg-black text-white">
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? 'bg-black/95 backdrop-blur-lg border-b border-white/10' : 'bg-black/50 backdrop-blur-sm'}`}>
        <div className="mx-auto px-6 flex items-center justify-between h-16">
          <Link to="/" className="flex items-center gap-2 group">
            <Shield className="h-7 text-white transition-transform group-hover:scale-110" />
            <span className="font-semibold">CYBER<span className="text-purple-500">DORK</span></span>
          </Link>
        </div>
      </nav>
      <main className="pt-20">{children}</main>
